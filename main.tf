...
jobs:
  build-and-deploy:
    ...
    env:
      # OCI CLI Variables are needed for the oracle-actions tools
      ...
      # Terraform Provider Input Variables
    ...
    steps:
      ...
      # This step creates the config file used by the OCI CLI and Terraform
      ...
      # ... (OCIR, Gradle, Docker steps are unchanged) ...
      ...
      # --- NEW FIX STEP ---
      # This installs the OCI command-line tool, which is required by the
      # Kubernetes provider's "exec" block to generate an auth token.
